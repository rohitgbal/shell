...
(select
 ...
 COALESCE(tblx01_gps_longitude_decimaldegrees_r_3, tblx01_advancedunshifted_longitudedecimaldegrees_r_3) as longitude,
 COALESCE(tblx01_gps_latitude_decimaldegrees_r_3, tblx01_advancedunshifted_latitudedecimaldegrees_r_3) as latitude,
 'CRC4T' as source,
 ...
 FROM change_database.change_table
  LATERAL VIEW explode(tblx01_wil_x) wilarray as wil where ((wil.code is not null and wil.code !='' and (SUBSTRING(wil.code,LENGTH(wil.code)-1,2)  <= 31 ))    and tblx01_partition_date_x >= date_sub(current_date,change_lookback))) QRY
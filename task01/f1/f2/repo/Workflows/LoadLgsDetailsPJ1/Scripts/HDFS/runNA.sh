...
spark-submit --master $SPARK_MASTER \
			 ...
			 --files 4gscript.sql \
             loader.py --database $DATABASE --sql 4gscript.sql --user $USER --table ntblx01_CRC4T_msg_pj1_sec_hte --output $OUTPUT --lookbacktable $DYS --region NA
			 

																																													
if [ "$?" != "0" ]; then
	 echo "*** WIL 4G NA Incremental Load Failed"
   exit 1
fi
exit 0																																													

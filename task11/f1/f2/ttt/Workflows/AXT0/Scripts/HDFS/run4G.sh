...
spark-submit --master $SPARK_MASTER \
			 ...
			 --files 4gscript.sql \
             loader.py --database $DATABASE --sql 4gscript.sql --user $USER --table $TABLE --output $OUTPUT --lookbacktable $DYS --current_date $CURRENT_DATE

if [ "$?" != "0" ]; then
	 echo "*** AXT 4G Incremental Load Failed"
   exit 1
fi
exit 0																																													

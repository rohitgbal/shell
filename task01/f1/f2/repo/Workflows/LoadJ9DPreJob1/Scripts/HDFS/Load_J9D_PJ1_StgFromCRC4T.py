...
database = sys.argv[1]
CRC4T_J9D_pj1_query_days = sys.argv[2]
output = sys.argv[3]


if __name__ == "__main__":
						
	spark = SparkSession.builder.appName("PACETransFormation_CRC4T_J9D_PJ1_STG").enableHiveSupport().getOrCreate()
	...
	FROM (SELECT ccrtbl.*, 'NULL' as tcu_strategy_part_number,Sha2(Concat(COALESCE(ccrtbl.vin,'NULL'),COALESCE(esn,'NULL'),COALESCE(correlationid,'NULL') \
	...
	FROM ctbl full outer join crtbl ON ctbl.vin=crtbl.vin \
        ...
        AND ctbl.cmdname IS NOT NULL \
	) ccrtbl ) ccrfinal where ccrfinal.vin IS NOT NULL")

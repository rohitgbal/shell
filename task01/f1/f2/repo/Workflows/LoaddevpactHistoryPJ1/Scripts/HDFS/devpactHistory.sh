...
spark-submit --conf spark.yarn.executor.memoryOverhead=$SPARK_YARN_EXECUTOR_MEMORY_OVERHEAD \
             ...
             --queue $OOZIE_QUEUE_NAME \
                         devpactHistory.py "$KERBEROS_KEYTAB_FILENAME" "$KERBEROS_PRINCIPAL" "$hive_database_name" "$outputPath" "$Incremental" "$source_history_table" "$raw_input_location" "$raw_inprocess_location" "$current_user" "$json_jar1" "$json_jar2" "$MKD_SCHEDULE_QUERY_DAYS" "$CURRENT_DATE"




if [ "$?" != "0" ]; then
        echo "*** devpactHistory to Staging Failed. STOPPING!!!"
        exit 1
fi

...
spark = SparkSession.builder.appName("AXTReload").enableHiveSupport().getOrCreate()
hive = HiveWarehouseSession.session(spark).build()
parser = argparse.ArgumentParser(description='AXTSummary Reload')
...
.replace("change_database",args.database).replace("change_table",args.source_table).replace("change_lookback",args.lookback).replace("current_date",args.current_date).replace("start_date",args.start_date)

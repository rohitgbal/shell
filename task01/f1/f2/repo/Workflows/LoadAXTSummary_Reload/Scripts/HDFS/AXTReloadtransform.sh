...
spark-submit --master $SPARK_MASTER \
			 ...
			 --files AXTReloadtransform_script.sql \
             AXTReloadtransform.py --database $DATABASE --sql AXTReloadtransform_script.sql --user $USER --source_table $SOURCE_TABLE --output $OUTPUT --lookbacktable $DYS --current_date $CURRENT_DATE --start_date $START_DATE

if [ "$?" != "0" ]; then
	 echo "*** AXTSummary Reload Failed"
   exit 1
fi
exit 0																																													

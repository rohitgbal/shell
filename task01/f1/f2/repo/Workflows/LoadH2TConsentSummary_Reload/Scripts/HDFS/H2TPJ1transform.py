...
spark = SparkSession.builder.appName("loadH2TDetailsPJ1_Reload").enableHiveSupport().getOrCreate()
hive = HiveWarehouseSession.session(spark).build()
parser = argparse.ArgumentParser(description='4G H2T PJ1 Historical Load')
...
parser.add_argument('--lkb_days',dest="lkb_days",required=True)
parser.add_argument('--curr_date', dest="curr_date", required=True)
parser.add_argument('--historyload', dest="history_load", required=True)
parser.add_argument('--temp_stg_table', dest="temp_stg_table", required=True)
parser.add_argument('--output', dest="output", required=True)

args = parser.parse_args()

select_temp= spark.sql("""select * from {0}.{1}""".format(args.database,args.temp_stg_table))

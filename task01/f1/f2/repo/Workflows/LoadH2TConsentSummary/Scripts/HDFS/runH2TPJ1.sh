...
spark-submit --master $SPARK_MASTER \
       ...
       --executor-memory $SPARK_EXECUTOR_MEMORY \
			 ...
			 --conf spark.sql.hive.hiveserver2.jdbc.url="jdbc:hive2://NNDhdp2-zk-1.NND.prdx.com:2181,NNDhdp2-zk-2.NND.prdx.com:2181,NNDhdp2-zk-3.NND.prdx.com:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2-ranger" \
       ...
       --conf spark.sql.hive.conf.list="hive.support.quoted.identifiers=none" \
        ...
        --conf spark.datasource.hive.warehouse.read.mode=DIRECT_READER_V1 \
			 H2TPJ1transform.py --database $DATABASE --user $USER --curr_date $CURRENT_DATE --lkb_days $DYS --temp_stg_table $TEMP_STG_TABLE --historyload $HISTORY_LOAD --output $OUTPUT


if [ "$?" != "0" ]; then
	 echo "*** H2T PJ1 Incremental Load Failed"
   exit 1
fi
exit 0
...
select a.vin,
    ...
    a.ufm_version,
    a.ufm_timestamp,
    if(c.save_region_code ='UNK' or c.save_region_code is null or trim(c.save_region_code) = ''
	or c.save_dlr_cntry_cd ='UNK' or c.save_dlr_cntry_cd is null
	or trim(c.save_dlr_cntry_cd) = '','UNKNOWN', c.save_region_code ) as partition_region,
	if(c.save_dlr_cntry_cd ='UNK' or c.save_dlr_cntry_cd is null or
	trim(c.save_dlr_cntry_cd) = '', 'UNKNOWN', c.save_dlr_cntry_cd) as partition_country
from(
	select tblx01_vin_d_3 as vin,
		...
		case when (sttg.consenttimestamp > tblx01_der_event_s_3) then tblx01_der_event_s_3
		when to_date(sttg.consenttimestamp) < date_sub('${current_date}',${lkb_days}) then tblx01_der_event_s_3
		else sttg.consenttimestamp end as consent_start_time,
		case when isnull(tblx01_policytableextensiontimestamp_s_3) then null
		else concat(tblx01_policytableextension_platformversion_r_3,'.',tblx01_policytableextension_majorversion_r_3,'.',
		tblx01_policytableextension_minorversion_r_3) end as pte_version,
		tblx01_policytableextensiontimestamp_s_3 as pte_timestamp,
		case when isnull(tblx01_userfriendlymessagestimestamp_s_3) then null
		else concat(tblx01_userfriendlymessages_platformversion_r_3,'.',tblx01_userfriendlymessages_majorversion_r_3,'.',
		tblx01_userfriendlymessages_minorversion_r_3) end as ufm_version,
	...
	and sttg.consenttimestamp is not null) a
  join ${hivevar:J9D_table} b ON a.vin = b.cvdcum_vin_d_3
  and (a.tblx01_cloud_msg_d = b.cvdcum_crltn_d_3 or a.tblx01_app_crltn_d_3 = b.cvdcum_app_crltn_d_3)
  ...
  left join ${hivevar:vin_meta_table} c on LPAD(a.vin,18,SUBSTR(a.vin,-1)) = c.vin;
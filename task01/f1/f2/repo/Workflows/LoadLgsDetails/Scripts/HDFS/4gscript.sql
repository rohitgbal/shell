...
(select
 ...
 FROM change_database.change_table
  LATERAL VIEW explode(tblx01_wil_x) wilarray as wil where ((wil.code is not null and wil.code !='' and (SUBSTRING(wil.code,LENGTH(wil.code)-1,2)  <= 31 )) and tblx01_partition_date_x >= date_sub('curr_date',change_lookback))) QRY
   where QRY.partition_date>=date_sub('curr_date',change_lookback)